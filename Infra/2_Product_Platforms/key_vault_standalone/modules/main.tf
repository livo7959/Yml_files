...
module "key_vault" {
  source = "../../../0_Global_Library/terraform-modules/key-vault"

  for_each = { for kv in var.key_vaults : "${kv.name}-${kv.environment}" => kv }

  resource_group                  = coalesce(each.value.resource_group, data.azurerm_resource_group.existing_rg[0].name)
