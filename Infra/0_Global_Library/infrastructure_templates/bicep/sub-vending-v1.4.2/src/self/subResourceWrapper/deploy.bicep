...
module networkSecurityGroup '../../carml/v0.11.1/network/network-security-group/main.bicep' = {
  ...
  name: 'nsg-${uniqueString(deployment().name, virtualNetworkLocation)}'
  params: {
    // name: 'nsg-${uniqueString(deployment().name)}'
    name: nsgName
    location: virtualNetworkLocation
    securityRules: (!empty(nsgSecurityRules)) ? nsgSecurityRules : []
    tags: (!empty(nsgTags)) ? nsgTags : {}
  }
}

module routeTable '../../carml/v0.11.1/network/route-table/main.bicep' = {
  scope: resourceGroup(subscriptionId, virtualNetworkResourceGroupName)
  name: 'route-${uniqueString(deployment().name, virtualNetworkLocation)}'
  params: {
    name: routeTableName
    location: virtualNetworkLocation
    routes: (!empty(routeTableRoutes)) ? routeTableRoutes : []
    tags: (!empty(routeTableTags)) ? routeTableTags : {}
  }
}
